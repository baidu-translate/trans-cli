...
description: >-
  Use this skill whenever you need to translate text with the trans-cli tool
  (Baidu Translation AI API). Covers JSON output contract, Baidu-specific
  language codes (jp/kor/fra/spa/ara — not ISO defaults), exit code semantics,
  error decision tree, first-time API key setup, QUOTA_EXCEEDED / AUTH_FAILED
  recovery, environment self-diagnosis via `trans doctor`, and listing supported
  languages via `trans languages`. Trigger on any of: running `trans text`,
  shell pipelines with translation, trans-cli errors, API key configuration,
  `trans doctor` checks, or looking up language codes.
...
metadata:
  clawdbot:
    ...
    install:
      - id: npm
        kind: npm
        package: "@bdtrans/trans-cli"
        bins:
          - trans
        label: Install trans-cli (npm)
---
...
Failing checks include `help_url`:
```json
{"name":"api_key","ok":false,"message":"not configured","help_url":"https://fanyi-api.baidu.com/manage/apiKey"}
```
...
Short-circuit: if `api_key` fails, the `connectivity` and `account` checks are skipped entirely
(the response contains only the `api_key` check object).
...
| 2 | `AUTH_FAILED` | Key invalid/expired (`raw_code` 52001/52002/52003) | Stop — run `trans doctor --json` then guide user to API key page |
| 3 | `QUOTA_EXCEEDED` | Balance = 0 (`raw_code` 54004) | Stop — guide user to recharge (see below) |
| 3 | `RATE_LIMITED` | QPS exceeded (`raw_code` 54003) | Wait 1 s, retry once |
